    ...
    \begin{minted}{python}
        class MViT(nn.Module):
            def __init__(self, skill_or_segment:str, modelinfo:dict, df_table_counts:pd.DataFrame):
                ...
                self.LastNNeurons = self._get_model_output_neurons(input_shape)
            ...
            def _get_model_output_neurons(self, shape):
                with torch.no_grad():
...
\begin{listing}
    \begin{minted}{python}
        ...
        for key, value in config.items():
            value_counts_train = train_generator.BalancedSet[ConfigHelper.lowerString(key)].value_counts(dropna=False)
            value_counts_val = val_generator.Skills[ConfigHelper.lowerString(key)].value_counts(dropna=False)
