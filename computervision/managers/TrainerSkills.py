...
class TrainerSkills:
    ...
    def validate(self, model, dataloader, device='cuda'):
        model.eval()
        val_loss = 0.0
        with torch.no_grad():
            for batch_X, batch_y in dataloader:
                ...
                # Compute losses as in training
                # (Optional: Track metrics like accuracy here)
    ...
    def train(self, modelname, from_scratch, epochs, save_anyway, unfreeze_all_layers=False, trainparams: dict= {}):
        try:
            ...
            for epoch in range(epochs):
                ...
                val_loss = self.validate(model=model, dataloader=dataloaderVal)
                print(f"Epoch {epoch+1}, Validation Loss: {val_loss / len(dataloaderTrain):.4f}")
