...
def get_model(modelinfo, df_table_counts: pd.DataFrame):
    ...
    x = Conv3D(filters=int(filters * 1.5), kernel_size=kernel_size, strides=2,
                  padding='same')(x)
    # skip = x
    x = SelfAttention()(x)

    # x = ConvLSTM2D(filters=int(filters * 1.5), kernel_size=kernel_size,
    #             padding='same', return_sequences=True)(x)
    # x = SelfAttention()(x) + skip
    
    x = ConvLSTM2D(filters=int(filters * 2), kernel_size=kernel_size, strides=2,
    ...
    x = Conv2D(filters // 4, kernel_size=1, activation='relu')(x)
    x = Flatten()(x)
