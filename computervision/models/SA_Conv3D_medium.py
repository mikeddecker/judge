...
import tensorflow as tf
from tensorflow.keras.layers import Layer, ConvLSTM2D, Conv2D, Conv3D, Activation, Flatten, Dense
...
def get_model(modelinfo, df_table_counts: pd.DataFrame):
    ...
    x = Conv3D(filters=int(filters * 1.5), kernel_size=kernel_size,
                  padding='same')(x)
    x = Conv3D(filters=int(filters * 2), kernel_size=kernel_size, strides=2,
                  padding='same')(x)
    
    x = SelfAttention()(x)

    x = Conv3D(filters=int(filters * 3), kernel_size=kernel_size, strides=1,
                  padding='same')(x)    
    x = Conv3D(filters=int(filters * 3), kernel_size=kernel_size, strides=2,
                  padding='same')(x)
    # skip = x
    x = SelfAttention()(x)
    
    x = ConvLSTM2D(filters=int(filters * 3), kernel_size=kernel_size, strides=2,
                  padding='same', return_sequences=False)(x)
    
    # Additional processing
    x = Conv2D(filters=int(filters*4), kernel_size=kernel_size, strides=2,
               activation='relu', padding='same')(x)
    x = Conv2D(filters=int(filters*5), kernel_size=kernel_size, strides=2,
               activation='relu', padding='same')(x)
    
    x = Conv2D(filters=int(filters*5), kernel_size=1, strides=2, activation='relu')(x)
    x = Flatten()(x)
