...
def get_model(modelinfo, df_table_counts: pd.DataFrame):
    ...
    x = ConvLSTM2D(filters=int(filters * 4), kernel_size=kernel_size, strides=2,
                  padding='same', return_sequences=False)(x)
    
    # Additional processing
    x = Conv2D(filters=int(filters*4), kernel_size=kernel_size, strides=1,
               activation='relu', padding='same')(x)
    x = Conv2D(filters=int(filters*5), kernel_size=kernel_size, strides=2,
               activation='relu', padding='same')(x)
    
    x = Conv2D(filters=int(filters*6), kernel_size=1, strides=2, activation='relu')(x)
    x = Flatten()(x)
