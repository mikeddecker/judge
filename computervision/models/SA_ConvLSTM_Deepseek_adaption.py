...
import tensorflow as tf
from tensorflow.keras.layers import Layer, ConvLSTM2D, Conv2D, Activation, Flatten, Dense
...
def get_model(modelinfo, df_table_counts: pd.DataFrame):
    ...
    x = ConvLSTM2D(filters=int(filters * 1.5), kernel_size=kernel_size, strides=2,
                  padding='same', return_sequences=True)(x)
    # skip = x
    x = SelfAttention()(x)
